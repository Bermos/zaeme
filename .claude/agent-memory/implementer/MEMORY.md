...
- 2026-09-14: the five guest-session and four non-owner checks in `scripts/api-smoke.sh` SKIP silently without `ZAEME_TEST_SESSION_COOKIE` / `ZAEME_TEST_GUEST_COOKIE`, and the twelve poster checks skip without `S3_BUCKET`. A "passed 94, failed 0" run has quietly proved 12 fewer things than a "passed 106" one — read the count, not the colour.
...
- 2026-09-14: the mail dry run's `[email:dry-run]` log prints the magic link twice — the `preview` field truncates the token, the `links` array does NOT. Either works for a human; a script should still read `zaeme_verification` rather than parse a log it may not be holding.
