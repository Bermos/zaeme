...
export interface BaseCurrencyHold {
  /** Every base currency expenses are recorded against, with its total. */
  bases: Array<{ baseCurrency: string, expenses: number }>
  /** Where those expenses are, so the owner knows what to act on. */
  events: Array<{ slug: string, title: string, baseCurrency: string, expenses: number }>
  total: number
}

/**
 * What holds the instance base currency in place, and where it lives (#25).
 *
 * `setInstanceBaseCurrency` refuses a change while any expense is recorded
 * against a different base, which on an instance with expenses makes the
 * setting effectively permanent: zäme has no admin-side expense surface, and
 * `removeExpense` is reachable only as a planner of that particular event. So
 * the owner needs to be told exactly WHICH events hold it rather than being
 * handed a count and left to guess — otherwise the only way out of a currency
 * they never chose is `psql`.
 *
 * Two statements, both grouped: no query per event.
 */
export async function baseCurrencyHold(): Promise<BaseCurrencyHold> {
  const db = useDb()
  const [bases, events] = await Promise.all([
    db.execute<{ base_currency: string, expenses: number }>(sql`
      select base_currency, count(*)::int as expenses
      from events_expense
      group by base_currency
      order by count(*) desc, base_currency
    `),
    db.execute<{ slug: string, title: string, base_currency: string, expenses: number }>(sql`
      select e.slug, e.title, x.base_currency, count(*)::int as expenses
      from events_expense x
      join events_event e on e.id = x.event_id
      group by e.slug, e.title, x.base_currency
      order by count(*) desc, e.slug
      limit 20
    `)
  ])
  const baseRows = bases.rows.map(r => ({ baseCurrency: r.base_currency, expenses: r.expenses }))
  return {
    bases: baseRows,
    events: events.rows.map(r => ({ slug: r.slug, title: r.title, baseCurrency: r.base_currency, expenses: r.expenses })),
    total: baseRows.reduce((sum, r) => sum + r.expenses, 0)
  }
}
