...
/**
 * Shared layout used by every transactional email. Keeps branding and
 * footer copy in one place.
 *
 * These templates are written with `React.createElement` (aliased as `h`)
 * rather than JSX because the Nuxt typecheck runs `vue-tsc --build`, which
 * assumes Vue JSX semantics for every `.tsx` file and rejects React JSX
 * config overrides on individual project references. Plain TS keeps the
 * types correct and the build working.
 */
